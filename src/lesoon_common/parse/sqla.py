...
TableType = t.Union[Table, Alias]


class OpParserABC(metaclass=abc.ABCMeta):
...
class LikeOpParser(OpParserABC):
    op_dict = {
        'like': 'like',
        'ilike': 'ilike',
        'notLike': 'not_like',
...
class OpParserFactory:
    parser_set: t.Set[t.Type[OpParserABC]] = {
...
def parse_multi_condition(
        ...
        models: t.List[TableType]) -> t.Tuple[SqlaExpList, SqlaExpList]:
    """多表过滤条件解析."""
    filter_list = list()
    order_list = list()
    for model in models:
...
def parse_filter(where: dict, model: TableType) -> SqlaExpList:
...
def parse_sort(sort_list: t.List[list], model: TableType) -> SqlaExpList:
    """排序解析.
    ...
    for sort_pair in sort_list.copy():
        ...
        if col is None:
            continue
        if (attr := parse_model_attribute(col, model)) is not None:
...
def parse_suffix_operation(column: str, value: t.Union[int, str, t.List[t.Any]],
                           model: TableType) -> t.Optional[SqlaExp]:
    if m := re.match(r'(?P<col>[\w\\.]+)_(?P<op>[\w]+)', column):
        col, op = m.group('col'), m.group('op')
        attr = parse_model_attribute(col, model)
    else:
        attr = parse_model_attribute(column, model)
...
def parse_valid_model_attribute(
    ...
    """
       ...
       Returns:
            attr: 字段名对应的Column实例对象
       """
    attr = parse_model_attribute(name=name, model=model)
    if not attr:
        raise ParseError(f'表模型: {model.__name__} 不存在列名为:{name}的列')
    return attr


def parse_related_models(statement: Select) -> t.List[TableType]:
    """获取Query对象查询涉及的所有表"""
    ...
    recur_related_models(_froms=statement.froms, _related_models=related_models)
